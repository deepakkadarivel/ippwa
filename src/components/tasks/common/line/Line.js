...
const renderLines = props => {
  ...
  return item.lines.map(x => {
    switch (x.type) {
      case 'text':
        return x.readOnly ? <FormText key={x.id} x={x} className='col-s-2 col-2 col-xs-3 Form-Field__line' handleChange={() => {}}/> :
          <FormInput key={x.id} x={x} className='col-s-2 col-2 col- Form-Field__line' handleChange={handleChange}/>;
      case 'select':
